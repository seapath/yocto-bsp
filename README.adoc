...
== Building a basic virtual machine
...
Yocto automatically generates a SBOM (SPDX files) and CVE annotations (OpenVEX file) in the directory `build/tmp/deploy/images/{machine-name}`:

* `{image-name}-{machine-name}.rootfs.json`, which contains annotations about some known CVEs (e.g. non-applicables CVEs due to a particular recipe configuration, outdated CVEs, etc.)
* `{image-name}-{machine-name}.rootfs.spdx.tar.zst`, which contains the whole SBOM of your distribution

To easily assess the vulnerabilities, we recommend to use https://github.com/savoirfairelinux/vulnscout/[Vulnscout] and https://github.com/bootlin/sbom-cve-check/{sbom-cve-check}, which are compatible with those types of files.

First you'll feed your SPDX and OpenVEX files to sbom-cve-check so it can detect applicable CVEs by looking at online databases. Then you'll use the exported file from sbom-cve-check as an input to Vulnscout so you can assess the pending vulnerabilities.

Below is an example of script to run sbom-cve-check and Vulnscout on files produced by a build of `host_efi`. It assumes you have cloned the Vulnscout repository at `~/Vulnscout` and that sbom-cve-check is available on your PATH.
[source,sh]
----
$ cd ~/Vulnscout
$ cp <path_to_yocto_build_dir>/tmp/deploy/images/seapath-hypervisor/seapath-host-efi-image-seapath-hypervisor.rootfs.{json,spdx.tar.zst} ./
$ sbom-cve-check \
  --sbom seapath-host-efi-image-seapath-hypervisor.rootfs.spdx.tar.zst \
  --yocto-vex-manifest seapath-host-efi-image-seapath-hypervisor.rootfs.json \
  --export-filter-vex-status affected not_affected under_investigation \
  --export-format yocto-cve-check-manifest \
  --export-path sbom-cve-checked.json
$ ./vulnscout.sh --name seapath-hypervisor \
  --spdx ./seapath-host-efi-image-seapath-hypervisor.rootfs.spdx.tar.zst \
  --cve-check ./sbom-cve-checked.json \
  --skip-grype-scan \
  --ignore-parsing-errors
----
We use the following flags in the `sbom-cve-check` command:

* `--export-filter-vex-status affected not_affected under_investigation` so we do not export `fixed` vulnerabilities since they represent the biggest part, are not useful for CVE analysis and would make the exported file much larger

* `--export-format yocto-cve-check-manifest` is to tell the util to create an OpenVEX file similar to the cve-check Yocto class. The ideal would have been to export an SPDX3 containing both the SBOM and the CVEs but we cannot do that since our input SBOM is an SPDX2 (this might change in a future update of sbom-cve-check).

Concerning the flags used in the `vulnscout.sh` command:

* `--skip-grype-scan` tells Vulnscout not to re-scan the SBOM (the .tar.zst file) to find new vulnerabilities since this is exactly what sbom-cve-check is doing. Furthermore, the CPEs generated by Yocto are missing some precision and make Grype produce a lot of false positives.

* `--ignore-parsing-errors` makes Vulnscout use a fast SPDX2 parser that greatly speeds up reading the huge SPDX archive produced by Yocto.

Wait for the analysis process to finish (it can take a few minutes) then connect to Vulnscout by using the URL shown in the command output.
