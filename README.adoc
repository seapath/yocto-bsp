...
The Yocto firmware generation has been tested on Ubuntu 22.04. You can either
...
For instance, for Seapath yocto-bsp project:
  ...
  $ cd my_project_dir/
  $ repo init -u https://github.com/seapath/repo-manifest.git -m <manifest.xml>
  $ repo sync
...
To build one of this flavor, run:
...
The build is started by running the following command:
  ...
  $ ./build.sh -i image -m boardname --distro distroname

You can also pass custom BitBake commands using the `--` separator:

  $ ./build.sh -i image -m boardname --distro distroname -- bitbake -c clean package_name

You can find below the Yocto images list:

* Host images
** host_docker_srg_imx8p_2gDDR: image with docker support for srg_imx8p_2gDDR AAEON machines
** host_docker_srg_imx8p_4gDDR: image with docker support for srg_imx8p_4gDDR AAEON machines
...
* Flasher images
** seapath-flasher: USB drive or PXE flash image used to flash efi SEAPATH images on
   disk.
during a PXE boot.
* Observer images
...
=== Flashing the firmware on AAEON srg-imx8p using a SD card

To be able to install Seapath on srg-imx8p, you need to use a SD card.
Suppose the location of your SD card is /dev/sdx and your machine is seapath-hypervisor-aaeon-2g

  $ sudo umount /dev/sdx*
  $ bmaptool copy --bmap sseapath-host-docker-srg-imx8p-seapath-hypervisor-aaeon-2g.wic.bmap seapath-host-docker-srg-imx8p-seapath-hypervisor-aaeon-2g.wic.gz /dev/sdx

As this does not reset u-boot env, it may be necessary to change the active partition.
On the u-boot interactive shell:
