...
The _SEAPATH_RM_WORK_ option can be set to `true` to enable the Yocto
`rm_work` class. This removes recipe work directories after they are built,
which significantly reduces disk usage during a build. It is enabled by
default in _seapath.conf.sample_.
...
Yocto automatically generates an SPDX3 file at `build/tmp/deploy/images/{machine-name}/{image-name}-{machine-name}.rootfs.spdx.json`. It contains:

* the whole SBOM of your distribution (which packages are installed, relationship between them and more)
* annotations about some known CVEs (e.g. non-applicables CVEs due to a particular recipe configuration, outdated CVEs, etc.)
...
First you'll feed your SPDX3 file to sbom-cve-check so it can detect applicable CVEs by looking at online databases. Then you'll use the exported file from sbom-cve-check as an input to VulnScout so you can assess the pending vulnerabilities.

Below is an example of script to run sbom-cve-check and VulnScout on the file produced by a build of `host_efi`. It assumes that both sbom-cve-check and vulnscout are available on your PATH.
[source,sh]
----
$ sbom-cve-check \
  --sbom <path_to_seapath_dir>/build/tmp/deploy/images/seapath-hypervisor/seapath-host-efi-image-seapath-hypervisor.rootfs.spdx.json \
  --export-filter-vex-status affected not_affected under_investigation \
  --export-format spdx3 \
  --export-path ./sbom-cve-checked.spdx.json
$ vulnscout --name seapath --variant host_efi \
  --add-spdx ./sbom-cve-checked.json \
  --serve
----
...
We use the `--export-filter-vex-status affected not_affected under_investigation` flag in the `sbom-cve-check` command so we do not export `fixed` vulnerabilities since they represent the biggest part, are not useful for CVE analysis and would make the exported file much larger.
