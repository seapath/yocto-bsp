...
* rte-image: production image to work with UEFI
* rte-bios-image: production image to work with BIOS/UEFI legacy
* rte-dbg-image: rte production image with debug tools
* rte-test-image: rte production image with test tools
* rte-guest-image: rte guest production image to work whith qemu
* rte-guest-dbg-image: rte guest production image with debug tools
* rte-guest-test-image: rte guest production image with test tools
...
USB key device is /dev/sdx:
  ...
  $ sudo umount /dev/sdx*
  $ gzip -d -c  build/tmp/deploy/image/boardname/rte-image-boardname.wic.gz | \
